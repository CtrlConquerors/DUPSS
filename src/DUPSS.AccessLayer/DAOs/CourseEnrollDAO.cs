...
namespace DUPSS.AccessLayer.DAOs
{
    public class CourseEnrollDAO : ICourseEnrollDAO
    {
        ...
        {
            _context.CourseEnrollments.Update(courseEnroll);
            await _context.SaveChangesAsync();
            return courseEnroll;
        }
        ...
        {
            var courseEnroll = await _context.CourseEnrollments.FindAsync(enrollId);
            if (courseEnroll == null)
                return false;

            _context.CourseEnrollments.Remove(courseEnroll);
            await _context.SaveChangesAsync();
            return true;
        }
    }
